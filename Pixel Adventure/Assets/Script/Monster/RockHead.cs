...
public class RockHead : Enemy
{
    ...
    void RockSummon()
    {
        if (summonnum == 1)
        {
            GameObject Summon1 = Instantiate(Rock, transform.position, transform.rotation);
            Rigidbody2D sr1 = Summon1.GetComponent<Rigidbody2D>();
            sr1.AddForce(Vector2.up * 8, ForceMode2D.Impulse);
        }
        if (summonnum == 2)
        {
            GameObject Summon2 = Instantiate(Rock, transform.position, transform.rotation);
            Rigidbody2D sr2 = Summon2.GetComponent<Rigidbody2D>();
            sr2.AddForce(Vector2.up * 8, ForceMode2D.Impulse);
        }
        if (summonnum == 3)
        {
            GameObject Summon3 = Instantiate(Rock, transform.position, transform.rotation);
            Rigidbody2D sr3 = Summon3.GetComponent<Rigidbody2D>();
            sr3.AddForce(Vector2.up * 8, ForceMode2D.Impulse);
        }
        summonnum++;
        if (summonnum > 3)
        {
